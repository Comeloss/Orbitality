...
namespace UniRx
{
        ...
        static IEnumerator Fetch(WWW www, IObserver<WWW> observer, IProgress<float> reportProgress, CancellationToken cancel)
        {
            using (www)
            {
                ...
                if (!string.IsNullOrEmpty(www.error))
                {
                    ...
                    var error = string.Format("Fetch: Can't load data from url {0}.\nError: {1}\n Bytes downloaded: {2}, " +
                                              "Text: '{3}', Headers: '{4}'"
        ...
        static IEnumerator FetchText(WWW www, IObserver<string> observer, IProgress<float> reportProgress, CancellationToken cancel)
        {
            using (www)
            {
                ...
                if (!string.IsNullOrEmpty(www.error))
                {
                    ...
                    var error = string.Format("FetchText:: Can't load data from url {0}.\nError: {1}\n Bytes downloaded: {2}, " +
                                              "Text: '{3}', Headers: '{4}'"
                        , www.url, www.error, www.bytesDownloaded, www.text, headers);
                    observer.OnError(new NetworkErrorException(www, error));
                }
        ...
        static IEnumerator FetchBytes(WWW www, IObserver<byte[]> observer, IProgress<float> reportProgress, CancellationToken cancel)
        {
            using (www)
            {
                ...
                if (!string.IsNullOrEmpty(www.error))
                {
                    ...
                    var error = string.Format("FetchBytes:: Can't load data from url {0}.\nError: {1}\n Bytes downloaded: {2}, Headers: '{3}'"
                        , www.url, www.error, www.bytesDownloaded, headers);
                    observer.OnError(new NetworkErrorException(www, error));
                }
                else
                {
    ...
    public class NetworkErrorException : Exception
    {
        ...
        public NetworkErrorException(WWW www, string text)
        {
            this.IsWWW = true;
            this.WWW = www;
            this.Url = www.url;
        ...
        public NetworkErrorException(string url, string text, int httpStatusCode)
        {
            this.IsWWW = false;
            this.Text = text;
            this.Url = url;
